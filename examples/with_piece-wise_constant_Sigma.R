...
fdr_vec        <- c(0.05, 0.1, 0.2) # target FDR level
    ...
    for (n_signif_X in n_signif_X_vec) {
      for (n_signif_Y in n_signif_Y_vec) {
        ...
        simulation_results <- foreach (iter = 1:num_iter, .combine = "cbind") %dopar% {
          ...
          CCA0 <- PMA::CCA(x = X0, z = Y0, typex = "standard", typez = "standard", K = 1,
                           trace = FALSE, penaltyx = lambda, penaltyz = lambda)
          u0 <- CCA0$u
          v0 <- CCA0$v
          ...
          # estimate covariance matrix of Y1tX1u0/sqrt(n1) (according to the asymptotic distribution)
          S_X0 <- cov(X0)
          S_Y0 <- cov(Y0)
          S_X0Y0 <- cov(X0, Y0)
          S_Y0X0 <- t(S_X0Y0)
          Omega <- matrix(NA, n_col_Y, n_col_Y)
          for (i in 1:n_col_Y) {
            for (j in 1:n_col_Y) {
