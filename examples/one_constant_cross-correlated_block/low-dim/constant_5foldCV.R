...
out_path <- paste0(out_path, "constant_low_dimensional_5foldCV/")
...
slurm_task_id  <- Sys.getenv("SLURM_ARRAY_TASK_ID") # run this as an array job 500/2=250 times
num_iter       <- 2
n_row          <- 3000
n_col_X        <- 50
n_col_Y        <- 50
  ...
  for (n_signif_Y in n_signif_Y_vec) {
    ...
    write_csv(simulation_results_df,
              path = paste0(out_path,
                            "constant_low_dimensional_5foldCV_",
                            slurm_task_id, ".csv"))
  }
}
