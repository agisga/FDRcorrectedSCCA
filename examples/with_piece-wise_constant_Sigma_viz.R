...
#    geom_segment(aes(x = 0, y = fdr, xend = max(n_signif_Y), yend = fdr), lty = 2) +
...
ggsave("./viz/constant_combined.pdf")
...
ggsave("./viz/constant_TP_combined.pdf")

# violin plot: number of true positives
simulation_results_df %>%
  filter(n_row == 500 & target_FDR == 0.1) %>%
  mutate(n_col = paste0("n = ", n_row, ", p = ", n_col_X, ", q = ", n_col_Y)) %>%
  mutate(n_col_vs_n_signif_X_vs_n_signif_Y = paste(n_col, n_signif_X,
                                                   n_signif_Y, sep = "_")) %>%
  mutate(n_col_vs_n_signif_X_vs_n_signif_Y = factor(n_col_vs_n_signif_X_vs_n_signif_Y,
               levels = rev(sort(unique(n_col_vs_n_signif_X_vs_n_signif_Y))),
               ordered = TRUE)) %>%
  mutate(n_signif_X = factor(paste0("s[X]==", n_signif_X))) %>%
  mutate(n_signif_Y = factor(paste0("s[Y]==", n_signif_Y))) %>%
  mutate(n_col = factor(n_col, rev(sort(unique(n_col))), ordered = TRUE)) %>%
  ggplot(aes(x = n_signif_Y, y = TP)) +
    geom_violin() +
    facet_grid(n_col ~ n_signif_X,
               labeller = labeller(n_signif_X = label_parsed)) +
    theme_bw()
...
simulation_results_df %>%
  ...
  ggplot(aes(sensitivity)) +
    ...
    facet_grid(n_col ~ n_signif_Y,
               labeller = labeller(n_signif_Y = label_parsed)) +
    theme_bw()

ggsave("./viz/constant_sensitivity_by_n_signif_Y.pdf")

# proportion of TP identified (combined for all n_signif conditions across the board)
simulation_results_df %>%
  filter(n_row == 500 & target_FDR == 0.1) %>%
...
ggsave("./viz/constant_sensitivity_hist.pdf")
